        ...
        <div class="blog_category_box_wrapper blog_box_wrapper2 float_left">
            ...
            <div class="lest_news_cont_wrapper float_left">
                <div class="blog_heaidng_top">
                    ...
                    <h3> <a href="{{ url('/competitions/competitionDetails/'.$competition->id) }}">{{ $competition->competition_name }}</a></h3>
                </div>
                <div class="blog-single_cntnt"> {!!mb_substr(html_entity_decode($competition->description),0,46) . ((strlen(html_entity_decode($competition->description)) > 46) ? '...' : '')!!} <a href="{{ url('/competitions/competitionDetails/'.$competition->id) }}"> read more</a> </div>
            </div>
        </div>
    </div>
    @endforeach
