...
class HomeController extends Controller
{
        ...
        if ($request->isMethod('post')) {
            ...
            if ($frontUser->save()) {            
                ...
                Mail::send('mails/register', $data, function ($message) use ($data) {                
                    $message->to('ateetagrawal@gmail.com', 'Artist Registration')->subject('A new Artist has registered');
    ...
    public function panelSignup(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($frontUser->save()) {
                ...
                Mail::send('mails/register', $data, function ($message) use ($data) {
                    $message->to('ateetagrawal@gmail.com', 'Panel Registration')->subject('A new Panel member has registered');
    ...
    public function fanSignup(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if ($frontUser->save()) {
                ...
                Mail::send('mails/register', $data, function ($message) use ($data) {
                    $message->to('ateetagrawal@gmail.com', 'Fan Registration')->subject('A new fan has registered');
                    $message->to($data['email'], 'Fan Registration')->subject('You have been registered successfully');
                    $message->from('xyz@gmail.com', 'Unsigned Star');
                });
                $verifyUser = VerifyUser::create([
                    'user_id' => $frontUser->id,
    ...
    {        
        ...
        if ($userDetails) {
            ...
            $userDetails->remainingDays = round(($datediff / (60 * 60 * 24)) + 1);
            return view('artist-dashboard')->with(compact('userDetails', 'agreementDetails','trackDetails','videoDetails','transactions', 'planDetails','photoDetails'));
        } else {
            return redirect('/');
        }        
    } 
    ...
    public function myProfile(Request $request, $id = null)
    {
        $dateTime = Carbon::now();
        $currentDate = $dateTime->toDateString();
        $userDetails = FrontUser::where(['id' => $id])->where('end_validity', '>=', $currentDate)->first();
        $top_songs = DB::table('tracks')
    ...
    public function uploadTrack(Request $request)
    {
        ...
        if ($uploadedtrack >= $paypalSubscribers->track_limit) {            
    ...
    public function songViewCounter(Request $request)
    {
        ...
        ->select('tracks.id', 'tracks.artist_id', 'tracks.track_name', 'tracks.track_file', 'tracks.cover_image', 'tracks.views', 'front_users.first_name', 'front_users.last_name')
        ->leftJoin('front_users', 'tracks.artist_id', '=', 'front_users.id')
        ->where('tracks.status', '=', '1')->where('front_users.status', '=', '1')
