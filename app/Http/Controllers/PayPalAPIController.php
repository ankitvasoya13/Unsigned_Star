...
class PaypalAPIController extends Controller
{
    ...
    public function SubscriberUpdate(Request $request)
    {
        ...
        $request = array(
            ...
            PaypalSubscribers::where(['user_id' => Session::get('userId')])->update([
                ...
                'next_billing_date'     => $subscriptions['billing_info']['final_payment_time'],
            ]);
                
            if ($subscriptions['status'] == 'ACTIVE') {
                FrontUser::where(['id' => Session::get('userId')])->update([
                    'end_validity' => $subscriptions['billing_info']['final_payment_time']
                ]);                
            }
    ...
    public function list_subscriber()
    {
        ...
        ->selectRaw('front_users.*, paypal_subscribers.plan_name, paypal_subscribers.last_payment_amount, paypal_subscribers.status')
        ->get();
